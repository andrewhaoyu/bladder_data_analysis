library(haven)
data <- read_sas("../data/bladderdiesel.sas7bdat", NULL)
View(data)
...
data$GRADE[is.na(data$GRADE)]=NA
data$STAGE[is.na(data$STAGE)]= NA
STAGE <- data$STAGE
idx <- which(is.na(STAGE)&data$CACO==1)
STAGE[idx] = 888
idx <- which(is.na(STAGE)&data$CACO==0)
STAGE[idx] = NA
table(STAGE,data$CACO,exclude = NULL)
GRADE <- data$GRADE
idx <- which(is.na(GRADE)&data$CACO==1)
GRADE[idx] = 888
idx <- which(is.na(GRADE)&data$CACO==0)
GRADE[idx] = NA
